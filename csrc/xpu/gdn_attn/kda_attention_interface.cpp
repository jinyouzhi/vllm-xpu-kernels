...
template <typename T>
void launch_kda_gated_delta_rule(
    sycl::queue& queue,
  ...
  const T* k_ptr = reinterpret_cast<const T*>(k.data_ptr());
  const T* v_ptr = reinterpret_cast<const T*>(v.data_ptr());
